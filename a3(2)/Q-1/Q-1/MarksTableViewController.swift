...
class MarksTableViewController: UITableViewController {
    ...
    var marks : [Stud_marks] = []
    
    func Disp() {
        let appD = UIApplication.shared.delegate as! AppDelegate
        let con = appD.persistentContainer.viewContext
        
        let fr = NSFetchRequest<Stud_marks>(entityName: "Stud_marks")
        
        do{
            marks = try! con.fetch(fr)
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = "RollNo : \(marks[indexPath.row].rollno) RDBMS :\(marks[indexPath.row].rdbms)"
        // Configure the cell...
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            let appD = UIApplication.shared.delegate as! AppDelegate
            let con = appD.persistentContainer.viewContext
            con.delete(marks[indexPath.row])
            try! con.save()
